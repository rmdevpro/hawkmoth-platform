# HAWKMOTH Enhanced Conversation Manager with LLM Routing (Updated)
import time
import json
import requests
from typing import Dict, Any, Optional
from git_handler import HAWKMOTHGitHandler, deploy_with_real_git, hawkmoth_self_commit
from llm_router import HAWKMOTHRouter, RoutingDecision, test_together_ai_connection
...
class EnhancedConversationManager:
    def __init__(self, analyzer):
        ...
        # API configurations (will be set from environment or config)
        ...
        self.routing_stats = {
            ...
            'routes_by_target': {},
            'total_cost': 0.0,
            'llm_routes': 0,  # Routes that used LLM vs rules
            'rule_routes': 0
        }
        
        # Test Together AI connection on startup
        self._test_together_connection()
    
    def _test_together_connection(self):
        """Test Together AI connection on startup"""
        try:
            result = test_together_ai_connection()
            if result["success"]:
                print("✅ Together AI connection verified")
            else:
                print(f"⚠️ Together AI connection failed: {result['error']}")
        except Exception as e:
            print(f"⚠️ Could not test Together AI connection: {e}")
    ...
    def process_message(self, user_id: str, message: str) -> Dict[str, Any]:
        ...
        # Track if this was an LLM-based or rule-based decision
        if hasattr(routing_decision, 'method'):
            if routing_decision.method == 'llm':
                self.routing_stats['llm_routes'] += 1
            else:
                self.routing_stats['rule_routes'] += 1
...
**Router Status**: LLM-based routing is now available for complex queries!

Try asking a complex question to see the LLM router in action."""
            else:
                return f"""❌ **Together AI Connection Test**
...
**Troubleshooting**:
• Check TOGETHER_API_KEY or TOGETHERAI_KEY in environment
• Verify API key is valid and has sufficient credits
• Check internet connectivity

**Current Status**: Falling back to rule-based routing only."""
        except Exception as e:
            return f"❌ Error testing Together AI: {str(e)}"
...
Together AI would handle this simple query cost-effectively.
**API Status**: {'✅ Configured' if self.router.together_api_key else '⚠️ Not configured'}

Use `test together` to verify API connection."""
    ...
    def _handle_routing_status(self) -> str:
        ...
        if router_info.get('api_key_source') != 'none':
            response += f"  └─ Key source: {router_info['api_key_source']}\n"
        ...
        response += f"• LLM Routes: {stats.get('llm_routes', 0)}\n"
        response += f"• Rule Routes: {stats.get('rule_routes', 0)}\n\n"
        ...
        response += "• `routing status` - This status display\n"
...
**The routing system is working** - this demonstrates the fallback mechanism!"""
    ...
    def _handle_hawkmoth_status(self):
        ...
        together_status = "✅ Configured" if self.router.together_api_key else "⚠️ Not configured"
...
• Full API integrations (Claude, GPT-4)
...
Use `routing status` to check LLM configuration and routing statistics."""
    ...
    def _handle_hawkmoth_commit(self):
        ...
        try:
            result = hawkmoth_self_commit("HAWKMOTH v0.1.0-dev - LLM routing system with Together AI")
            if result['success']:
...
HAWKMOTH now includes intelligent routing to specialized LLMs with Together AI integration."""
            else:
...
**New**: Try `test together` to verify Together AI integration!

Try: `hawkmoth status` or `routing status` or paste a GitHub URL!"""
