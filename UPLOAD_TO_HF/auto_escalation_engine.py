...
# Import Google Search integration
try:
    from google_search import HAWKMOTHGoogleSearch
    GOOGLE_SEARCH_AVAILABLE = True
except ImportError:
    GOOGLE_SEARCH_AVAILABLE = False
    print("⚠️ Google Search not available - install google-api-python-client")
...
class HAWKMOTHAutoEscalationEngine:
    def __init__(self):
        ...
        self.escalation_chains = {
            'real_time_data': ['WEB_SEARCH'],  # Date/time queries go directly to web search
            'current_date': ['WEB_SEARCH'],   # Direct date queries
        ...
        if GOOGLE_SEARCH_AVAILABLE:
            self.google_search = HAWKMOTHGoogleSearch()
        ...
        print(f"   • Google Search: {'✅ Available' if self.google_search and self.google_search.search_available else '⚠️ Not configured'}")
        ...
        for pattern in self.failure_patterns:
            if re.search(pattern.lower(), response_lower):
                return EscalationDecision(
                    ...
                    confidence=0.95,
                    estimated_cost=0.005  # Google Search cost
                )
    ...
    def _execute_web_search(self, query: str) -> Dict[str, Any]:
        """Execute web search escalation with real Google Search"""
        
        # Special handling for date queries (local is faster)
        if any(pattern in query.lower() for pattern in ['date', 'today', 'current date', 'what is the date']):
        ...
        # Special handling for time queries (local is faster)
        if any(pattern in query.lower() for pattern in ['time', 'current time', 'what time is it']):
        ...
        # Use Google Search for all other queries
        if self.google_search:
            print(f"🔍 Executing Google Custom Search: {query}")
            search_result = self.google_search.search_with_fallback(query, num_results=3)
            return search_result
        else:
            # Fallback if Google Search not available
            return {
                'success': True,
                'response': f"🔍 **Web Search Needed**: {query}\n\n⚠️ Google Custom Search not configured. Set GOOGLE_API_KEY and GOOGLE_CSE_ID environment variables to enable real web search.",
    ...
    def get_escalation_stats(self) -> Dict[str, Any]:
        ...
        # Add Google Search stats if available
        if self.google_search:
            stats.update({
                'google_search_stats': self.google_search.get_search_stats()
            })
