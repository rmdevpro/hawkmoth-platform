...
class HAWKMOTHStickySessionEngine:
    ...
    def continue_conversation(self, session_id: str, user_message: str) -> tuple[LLMResponse, bool]:
        """Continue conversation in existing session with failure detection and auto-escalation"""
        ...
        if switch_decision.requires_switch:
            ...
            response = self.execute_with_current_model(session, user_message)
            
            # Check if response indicates failure and auto-escalate
            escalation_needed, escalated_response = self.check_and_escalate(session, user_message, response)
            
            if escalation_needed:
                return escalated_response, True  # Model was switched due to escalation
            else:
                return response, False  # No switch needed
    ...
    def route_initial_query(self, user_message: str) -> RoutingDecision:
        ...
        date_time_keywords = ['date', 'time', 'today', 'now', 'current date', 'current time']
        if (any(keyword in message_lower for keyword in hawkmoth_keywords) or
            any(keyword in message_lower for keyword in date_time_keywords)):
            return RoutingDecision(
                target_llm='HAWKMOTH_LOCAL',
                model_config=self.model_catalog['HAWKMOTH_LOCAL'],
                confidence=0.95,
                reason='Platform command or date/time query - local processing',
                estimated_cost=0.0,
                complexity='simple'
            )
        ...
        simple_patterns = ['what is', 'how to', 'explain', 'define', 'tell me about', 'what', 'when', 'where', 'who', 'why', 'how']
...
"""
    ...
    def _execute_hawkmoth_local(self, message: str, session: ConversationSession) -> LLMResponse:
        ...
        if any(keyword in message_lower for keyword in ['date', 'time', 'today', 'now']):
            from datetime import datetime
            now = datetime.now()
            if 'time' in message_lower:
                content = f"Current time: {now.strftime('%I:%M %p %Z')}"
            else:
...
🛣️  Available Lanes:
   ...
   • Premium Analysis: $3/$15/1k (Claude Sonnet 4)"""
    ...
    def check_and_escalate(self, session: ConversationSession, user_message: str, initial_response: LLMResponse) -> tuple[bool, LLMResponse]:
        """Check if response indicates failure and auto-escalate to better model"""
        response_lower = initial_response.content.lower()
        
        # Failure patterns that indicate model cannot handle the query
        failure_patterns = [
            "i don't have access",
            "i can't check", 
        ...
        escalation_target = self.determine_escalation_target(session, user_message)
        
        if not escalation_target:
            return False, initial_response
        
        print(f"\n🔄 Auto-Escalation Triggered")
        print(f"   From: {session.primary_model}")
        print(f"   To: {escalation_target}")
        print(f"   Reason: Detected failure pattern")
        
        return self.execute_escalation(session, user_message, escalation_target)
    
    def determine_escalation_target(self, session: ConversationSession, user_message: str) -> str:
        """Determine escalation target based on query and current model"""
        message_lower = user_message.lower()
        current_model = session.primary_model
        
        # Real-time data queries → Claude (better at explaining limitations)
        realtime_keywords = ['win', 'won', 'lose', 'lost', 'score', 'game', 'news', 'yesterday', 'last night', 'current', 'latest']
        if any(keyword in message_lower for keyword in realtime_keywords):
            if current_model != 'CLAUDE_SONNET_4':
                return 'CLAUDE_SONNET_4'
        
        # Default escalation chain
        escalation_chain = {
            'DEEPSEEK_R1_FREE': 'DEEPSEEK_V3',
            'DEEPSEEK_V3': 'CLAUDE_SONNET_4',
            'DEEPSEEK_R1': 'CLAUDE_SONNET_4'
        }
        
        return escalation_chain.get(current_model)
    
    def execute_escalation(self, session: ConversationSession, user_message: str, target_model: str) -> tuple[bool, LLMResponse]:
        """Execute escalation to target model"""
        if target_model not in self.model_catalog:
            return False, None
        
        # Switch to target model
        old_model = session.primary_model
        session.primary_model = target_model
        session.model_config = self.model_catalog[target_model]
        
        try:
            # Execute with escalated model
            escalated_response = self.execute_with_current_model(session, user_message)
            return True, escalated_response
            
        except Exception as e:
            # Restore original model on failure
            session.primary_model = old_model
            session.model_config = self.model_catalog[old_model]
            return False, None
